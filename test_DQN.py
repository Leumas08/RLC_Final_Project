...
from stable_baselines3 import DQN as alg
import cloudpickle
import torch
...
model = alg("MlpPolicy", env, verbose=1, tensorboard_log = "DQN1k/").learn(total_timesteps=1000)
...
for i in range(100):
    ...
    if done:
        obs = env.reset()
        i = 100
        torch.save(model.policy.state_dict(), 'DQN_1k.pt')
env.close()
