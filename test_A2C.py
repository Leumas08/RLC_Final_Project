...
from stable_baselines3 import A2C as alg
import torch
...
model = alg("MlpPolicy", env, verbose=1, tensorboard_log = "A2C1k/").learn(total_timesteps=1000)
...
for i in range(100):
    ...
    if done:
        obs = env.reset()
        i = 100
        torch.save(model.policy.state_dict(), 'A2C_100k.pt')
env.close()
